import numpy as np
import warnings
from pyinsurance.metrics.metrics import (
    Sharpe_rat,
    Sortino_rat,
    Omega_rat,
    Modigliani_rat,
...
from pyinsurance.metrics.returns_metrics import cagr, Cumulative_ret
from pyinsurance.metrics.probabilistic_sharp_ratio import probabilistic_sharpe_ratio
from pyinsurance.metrics.std import stdeviation
from pyinsurance.portfolio.tipp import TIPP
from pyinsurance.portfolio.Drawdown import Drawdown_function
from pyinsurance.stats.jensen import jensen_alpha_beta
...
class tipp_model:
    ...
    def __init__(
        ...
        lock_in,
        Min_risk_part,
        Capital_reinjection_rate,
        Initial_funds,
        floor_percent,
        multiplier,
        Benchmark_returns,
        Rebalancement_frequency=252,
        theta=0.01,
        Horizon=1,
    ) -> None:
        ...
        self.Sharpe_ratio = Sharpe_rat(
            self.return_mtx, self.safe_asset_mtx, Rebalancement_frequency
        )

        self.Sortino_ratio = Sortino_rat(
            self.return_mtx, self.safe_asset_mtx, Rebalancement_frequency
        )

        self.Breadth = Information_rat(
            self.return_mtx, self.Benchmark_returns, Rebalancement_frequency
        )

        self.PSR = probabilistic_sharpe_ratio(
            self.return_mtx,
        ...
        self.Modigliani_ratio = Modigliani_rat(
            self.return_mtx,
            self.Benchmark_returns,
            self.safe_asset_mtx,
            Rebalancement_frequency,
        ...
        self.Beta, self.Alpha = jensen_alpha_beta(
            self.return_mtx, self.Benchmark_returns, Rebalancement_frequency
        )
    ...
    def Runner(
        ...
        (
            self.Fund,
            self.floor,
            self.capital_reinjection,
            self.Reference_capital,
            self.risk_returns_mtx,
            self.safe_asset_mtx,
            self.return_mtx,
        ) = TIPP(
            risk_returns,
